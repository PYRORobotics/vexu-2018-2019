 ...
 */
/*
PYROIMUGyro::PYROIMUGyro() : pros::ADIGyro(-1)
{

}

double PYROIMUGyro::value = 0;

double PYROIMUGyro::get_value(void) const
{
  return value = HEADING_ADJUSTED * 10;//+=0.00001;//0.0123456789;
}

PYROIMUGyro IMUGyro;

PYROOkapiIMUGyro::PYROOkapiIMUGyro(std::uint8_t iport, double imultiplier) : ADIGyro(-1), gyro(IMUGyro)
{
  gyro = IMUGyro; // Just in case?
}

PYROOkapiIMUGyro::~PYROOkapiIMUGyro()
{}

double PYROOkapiIMUGyro::get() const
{
  return IMUGyro.get_value(); //IMUGyro.get_value()
}

double PYROOkapiIMUGyro::controllerGet()
{
  return get();
}

std::int32_t PYROOkapiIMUGyro::reset()
{
  //Maybe reset here?
}


PYROOkapiIMUGyro OkapiIMUGyro(-1);
*/
/*
 class PYROChassis
 {
